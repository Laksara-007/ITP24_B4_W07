...
export const inventoryStats = async (req, res) => {
    try {
        const response = await getReleasePerDayService();
        if (!response) return makeResponse({ res, status: 500, message: 'Process failed... Something went wrong' });
        makeResponse({ res, status: 200, data: response, message: 'Successfully retrived data' });
    } catch (error) {
        throw new badRequest(error.message);
    }
};

export const requestPerItems = async (req, res) => {
    try {
        const response = await requestPerItemService();
        if (!response) return makeResponse({ res, status: 500, message: 'process failed... Something went wrong' });
        else makeResponse({ res, status: 200, data: response, message: 'Successfully retrieved data' });
    } catch (error) {
        throw new badRequest(error.message);
    }
};
