...
export const getReleasePerDay = async () => {
    try {
        const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);

        const response = await releaseStocksModel.aggregate([
            {
                $match: { createdAt: { $gt: sevenDaysAgo } }
            },
            {
                $group: { _id: '$departmentName', totalQuantity: { $sum: '$quantity' } }
            }
        ]);
        return response;
    } catch (error) {
...
export const requestPerItemRepo = async () => {
    try {
        const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
        const response = await releaseStocksModel.aggregate([
            {
                $match: {$and: [{createdAt: { $gt: sevenDaysAgo } }]}
            },
            {
                $group: { _id: '$stockId', totalQuantity: { $sum: '$quantity' } }
            }
        ]);
        return response;
    } catch (error) {}
