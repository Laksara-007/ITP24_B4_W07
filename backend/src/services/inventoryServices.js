...
import {
    ...
    updateInventory,
    updateStockDetails
} from '../repositary/inventoryRepo.js';
...
export const getReleasePerDayService = async () => {
    try {
        const response = await getReleasePerDay();
        return response;
    } catch (error) {
        console.log(error);
    }
...
export const requestPerItemService = async () => {
    try {
        let response = await requestPerItemRepo();

        let itemR;
        response = await Promise.all(
            response.map(async (item) => {
                itemR = await getInventoryByID(item._id).then((res) => {
                    item._id = res.inventoryName;
                    item.type = res.type;
                });
                return item;
            })
        ...
        return response;
        // return response.filter((res) => res.type === type);
    } catch (error) {
        console.log(error);
    }
};
